...
- when an AC applies to multiple consumer types with different integration patterns, preserve the per-consumer pass condition rather than abstracting to a generic concept (e.g., "Skill References table row" for commands vs "## Section heading" for reviewer prompts — not "references the skill")
