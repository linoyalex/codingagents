...
function parseSkillReferences(commandText, commandName) {
  ...
  for (const line of sectionText.split('\n')) {
    ...
    if (cols.length >= 2) {
      const skill = cols[0];
      const sourcePath = cols[1];

      if (!sourcePath) {
        throw new Error(
          `Command '${commandName}': Skill References table has blank Source path for skill '${skill}'`
        );
      }

      rows.push({ skill, sourcePath });
    }
  }
...
function parseRequiredArtifacts(skillText, skillName) {
  ...
  for (let i = headerIdx + 2; i < lines.length; i++) {
    ...
    const pathsRaw = cols[2].split(',').map(p => p.trim()).filter(Boolean);

    if (!pattern) {
      throw new Error(
        `Skill '${skillName}': malformed Required Artifacts table -- blank Pattern cell in data row: '${line}'`
      );
    }
    if (pathsRaw.length === 0) {
      throw new Error(
        `Skill '${skillName}': malformed Required Artifacts table -- blank Path cell in data row: '${line}'`
      );
    }
