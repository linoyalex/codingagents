...
commands/implement.md                      # +1 verification step (AC4)
commands/test-design.md                    # +1 verification step (AC5)
```
...
**Stage 1 -- Discovery.** Glob `commands/*.md`. For each command file, find lines matching
`Read .claude/skills/<name>/SKILL.md` (or the source path `skills/<name>/SKILL.md`).
...
**Stage 3 -- Wiring check.** For each artifact in the registry, search the invoking command's
full text for: (a) the Pattern value (substring or escaped regex match), and (b) at least one
Path value (AC9 -- multiple paths pass if any one matches). If either is missing, fail with a
message: `"Skill '<skill>' requires artifact '<artifact>' (pattern: <pattern>, path: <path>) but command '<command>' does not reference it"` (AC1, AC2, AC8).
...
| Command missing artifact wiring | Stage 3 pattern/path check | Test fails naming skill, command, artifact, and what is missing (AC1) |
...
| AC8 | Stage 3 checks mention regardless of Condition column |
