...
- **RCA — symmetric coverage gap in this PRD:** This PRD itself exhibited the defect class ISS-043 describes — command-side changes got contract test ACs but skill-side changes did not. The `/specify` command does not instruct the product-owner to verify symmetric test coverage across all artifact locations mentioned in ACs. This gap is noted for future `/specify` hardening.
